...
function EditableEmployeeId({
  ...
  const [value, setValue] = useState(user.employeeId)
  const inputRef = useRef<HTMLInputElement>(null)
  const [state, formAction, isPending] = useActionState<
    UpdateEmployeeIdState,
    FormData
  ...
  useEffect(() => {
    ...
    if (state?.errors?.employeeId) {
      toast.error(state.errors.employeeId[0])
    }
  ...
  if (!editing) {
    return (
      <div className="flex items-center gap-1 group">
        ...
        {!isCurrentUser && (
          <button
            onClick={() => { setValue(user.employeeId); setEditing(true) }}
        ...
        {users.map((user) => (
          <TableRow key={user.id}>
            <TableCell className="font-medium">
              <EditableEmployeeId user={user} isCurrentUser={user.id === currentUserId} />
            </TableCell>
